...
```bash
# 统计所有 .py 中的 C++ 提及
grep -rn "C++" projects/python/tutorials/*.py
```
...
| C++对比基线 | `projects/python/tutorials/*.py` (46处已标记不可改) |
...
```bash
# C++ 对比统计
grep -rn "C++" projects/python/tutorials/*.py | grep -v "无 C++ 对比"

# 练习区扫描
grep -rn "# ↓ 你的代码 ↓" projects/python/tutorials/*.py
