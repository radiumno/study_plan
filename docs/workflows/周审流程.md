...
```bash
# 核对当前进度
grep -n "Day\|已完成\|当前状态" resources/主脉络.md docs/plans/阶段*_分脉络大纲.md vault/会话交接.md
```

检查清单:
- [ ] 主脉络进度 vs 细脉络进度 vs 实际文件数 三者一致
- [ ] `vault/会话交接.md` 的最新状态与待办是最新的
- [ ] 细脉络中当前 Day 的标记正确 (✅/⏳/▶)
...
```bash
# 检查每个练习区是否被预填了答案
grep -A2 "# ↓ 你的代码 ↓" projects/python/stage*/day*.py
```
...
# 进度检查
head -40 vault/会话交接.md | grep "已完成\|状态"
