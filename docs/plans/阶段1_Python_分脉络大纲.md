...
**协作模式：** Hermes 写计划 → Claude Code 检查依赖 + 答疑讲题。见 `COLLEAGUES.md`
...
│   ├── Day 03: 函数、作用域、lambda  ⚠️（模板完成，练习未填）
