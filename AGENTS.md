...
4. `resources/核心学习资源清单.md`
5. `docs/plans/规划层级与滚动执行规则.md`
6. `docs/workflows/写课程流程.md`
7. `vault/会话交接.md`
...
4. `docs/plans/技能需求-权重矩阵.md`
5. `resources/核心学习资源清单.md`
6. `resources/lib/资源库.md`
7. `docs/plans/规划层级与滚动执行规则.md`
8. `docs/workflows/写脉络流程.md`
9. `vault/会话交接.md`
...
- 课程状态统一为：计划中 -> 课程已写 -> 学习中 -> 待Review -> Review通过 / 返工。
- 课程文件写完不等于用户学完。
- 只有用户练习经过 Review 并达到验收标准，阶段计划才能标记“Review通过”。
- 实时进度优先级：用户实际练习与Review结果 > `vault/会话交接.md` > 当前阶段细脉络 > 主脉络。
...
- 超前内容只允许出现在“选做”题
- 用户已经填写的练习不是“污染”，不得自动覆盖或清空
- 只有需要重新生成干净教学模板时，才先归档，再运行 `python3 scripts/archive_and_clear_filled_exercises.py stage1|stage2`
...
- 写课前至少参考 2 个来源
- 优先读取 `resources/核心学习资源清单.md`
- 优先顺序：官方文档 / 原始教程 > 核心资源清单 > 候选资源库 > 社区文章或视频
- 在课程文件顶部标注参考源

## 写脉络硬规则

- `resources/主脉络.md` 是长期总规划
- `docs/plans/阶段X_分脉络大纲.md` 是阶段内执行规划
- 两者不一致时，先修规划，再写课
- 做阶段规划时,必须先做学习量/容量预算: 阶段终点、总可用时间、主线比例、缓冲比例、每日上限、验收方式
- 当前阶段必须细到Day；下一阶段只细化未来1-2周；更远阶段保留Block、项目和闸门，避免伪精确
- 阶段2以后进入新Block或新week前，必须滚动拆出未来1-2周的day-by-day微计划
- AI没有固定时间配额；只有主线当期任务通过后才能占用剩余容量
- 做阶段规划时，必须显式检查：
  - 是否符合量化开发主线
  - 是否对应岗位需求
  - 是否存在前置依赖断裂
...
- 招聘调研先按岗位族拆分，不把交易系统、研究平台、量化研究和AI基础设施混成一个平均岗位
- 没有逐条岗位样本表时，不写“岗位出现率”“典型薪资”“市场中位数”
- star、播放量和“最新版”不能直接决定资源优先级

如果发现高价值资源：

1. 判断是否与当前学习计划相关
2. 先更新 `resources/lib/资源库.md` 作为候选
3. 只有明显优于现有主资源时，才更新 `resources/核心学习资源清单.md`
4. 如果能立刻影响写课或规划，再同步更新 workflow 或阶段计划
...
- `vault/会话交接.md`
- 主脉络通常不维护逐日进度，只有阶段目标或路线改变时才更新
- 运行 `python3 scripts/checks.py plans`
